...
function startGL()
{
  ...
  const vertextShaderSource = ` //Znak akcentu z przycisku tyldy - na lewo od przycisku 1 na klawiaturze
    ...
    uniform mat4 uMatrixRotX;
    uniform mat4 uMatrixRotY;
    uniform mat4 uPMatrix;
    void main(void) {
      mat4 uMatrix = uMatrixRotX * uMatrixRotY;
  ...
  const fragmentShaderSource = `
    void main(void) {
       gl_FragColor = vec4(0.0,0.0,1.0,1.0); //Ustalenie stałego koloru wszystkich punktów sceny
    }
  `;
  ...
  let angle = 19.0; //Macierz transformacji świata - określenie położenia kamery 

  let uMatrixRotZ = [
  Math.cos(angle*Math.PI/180.0),-Math.sin(angle*Math.PI/180.0),0,0, //Macierz Rotacji z
  ...
  let uMatrixRotX = [
  1,0,0,0,
  0,Math.cos(angle*Math.PI/180.0),-Math.sin(angle*Math.PI/180.0),0, //Macierz Rotacji x
  0,Math.sin(angle*Math.PI/180.0),Math.cos(angle*Math.PI/180.0),0.0,
  0,0,-5,1 //Położenie kamery
  ];

  let uMatrixRotY = [
  Math.cos(angle*Math.PI/180.0),0,Math.sin(angle*Math.PI/180.0),0, //Macierz Rotacji y
  0,1,0,0,
  ...
  gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMatrixRotX"), false, new Float32Array(uMatrixRotX));
  gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, "uMatrixRotY"), false, new Float32Array(uMatrixRotY));
